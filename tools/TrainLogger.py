...
class TrainLogger:
    ...
    def epoch_init(self, training=True):
        self.loss, self.count, self.pred, self.gt, self.label_partseg = 0., 0., [], [], []
        if training:
    ...
    def step_update_partseg(self, pred, gt, loss, label_partseg, training=True):
        if training:
            self.step += 1  # Use TensorFlow way to count training steps
        self.gt.append(gt)
        self.pred.append(pred)
        self.label_partseg.append(label_partseg)
        batch_size = len(label_partseg)
        self.count += batch_size
        self.loss += loss * batch_size
    def calculate_shape_iou(self):
        label = self.label_partseg.squeeze()
        seg_np = self.gt
        pred_np = self.pred
        shape_ious = []
        ...
        # print()
        self.logger.info([np.mean(item) for item in cat_ious])
        return shape_ious
    def epoch_update(self, training=True, mode='cls'):
        ...
        if mode == 'partseg':
            instance_acc = metrics.accuracy_score(self.gt.reshape(-1,1), self.pred.reshape(-1,1))
        else:
        ...
        if mode == 'cls':
            ...
            self.label_partseg = np.concatenate(self.label_partseg)
            miou = np.mean(self.calculate_shape_iou())
            if miou > self.best_miou and not training:
                self.best_miou_epoch = self.epoch
                self.save_model = True
                self.best_miou = miou
